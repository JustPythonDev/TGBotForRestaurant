...
def show_cart(chat_id):
    cursor.execute("SELECT item_name, price FROM cart WHERE user_id=?", (chat_id,))
    items = cursor.fetchall()

    total_price = sum(item[1] for item in items)
    items_list = "\n".join([f"{item[0]} - {item[1]} руб." for item in items])
...
def cash_payment(call):
    ...
    cursor.execute("SELECT total_price FROM orders WHERE user_id=?", (chat_id,))
