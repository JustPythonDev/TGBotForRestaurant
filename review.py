...
def cart_menu_start(callback, user_id):
    ...
    if callback == ('set_review'):
        review_messages = set_review(callback, user_id)
    if callback == ('view_reviews'):
        review_messages = view_reviews(callback, user_id)
...
def view_reviews(callback, user_id):
...
def set_review(callback, user_id):
    review_messages = []
    buttons = []
    buttons = [
